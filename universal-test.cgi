...
sub start_session
{
  ...
  my $fwconfigAMT = $q->param('FWconfig');
...
if ($q->param('Action') =~ m/^Load/)
{
  ...
  if (defined $file)
  {
  #  start_session($file); AMT
    start_session($fn_str);
  }
  else
  {
...
else
{
  ...
  print"<h2 style=\"color:navy\">Universal data format @ $shost </h2>";
