...
void rdf()
{
   ...
   auto muonHelper = node.Book(CandHelper(d.GetNSlots()), {"nMuon", "Muon_pt", "Muon_eta", "Muon_phi"});
   ...
   vsdt.append_collections({"cands", "jets", "primvs", "cmets", "infos"});
   ...
   for (int e = firstEvent; e < lastEvent; ++e)
   {
      ...
      }
      {
         ...
         for (auto vsd : vsdc.m_list) {
            ...
            vsdt.cmets().emplace_back(m->pt(), m->eta(), m->phi(), m->sumEt());
            m->dump();
         }
      }
      {
